...
**H1 裁定的适用范围 caveat**（同样在图 4-1 图注中声明）：H1 的裁定依据是 `ABL_B2_adv_adaptive` 这**一次** 150 epoch 训练（seed 20260811）。它确立的是"在该次训练内，两分支梯度范数比是非平稳的"，**不构成跨随机种子可复现性的证据**。本文全文不报告跨 seed 方差（理由见第 5 章对样本量与实验范围的说明），故 H1 应读作"该机制在本文实测条件下确实被触发"，而非"该机制在任意初始化下必然呈现同一量级的演化"。
...
1. $\hat\rho$ 跨 150 epoch 演化跨越 **3.92×10⁶ 倍**（最小值 0.00576313、最大值 22613.8），平台区 [75, 149] n=75 的中位数为 2212.65，p10 为 391.997、p90 为 13161.7；
...
**极差的稳健性核验。** 仅报告 $\hat\rho_{\max}/\hat\rho_{\min}$ 会面对一项合理质疑：极差是两个端点之比，单个异常 epoch 即可将其推高若干数量级，因此极差本身不能区分"阶段性演化"与"离群点扰动"。为排除该解释，本文对同一轨迹补充三项稳健性核验（脚本 `scripts/_verify_h1_robustness.py`，判据跑前定死）。

第一，**抗异常点极差**。剔除首尾各 10% 分位后，$\hat\rho_{p90}/\hat\rho_{p10} = 4.31×10^5$ 倍；剔除首尾各 5% 分位后为 $1.51×10^6$ 倍。即使删去两端各 15 个 epoch，跨度仍远超 H1 接收条件所要求的 100 倍。

第二，**阶段性演化**。早期区间 [0, 25) 的 $\hat\rho$ 中位数为 0.0205，平台区 [75, 150) 为 2212.65，两者之比 $1.08×10^5$ 倍。该比值由两组各含 25 与 75 个 epoch 的中位数构成，不依赖任何单点取值。

第三，**单调性**。将 150 epoch 等分为 6 段（每段 25 epoch），各段中位数依次为 0.0205、30.04、228.2、493.7、1778、11624，段序与段中位数的 Spearman 秩相关系数为 **1.0000**（严格单调上升）。图 4-1 上图以橙色阶梯线叠加该分段中位数，使单调趋势不依赖对细部曲线的判读。

三项核验一致表明，$\hat\rho$ 的跨数量级变化是**训练全程的单调演化**，而非离群 epoch 造成的假象。因此 H1 的成立不依赖极差这一单一统计量。

该图直接支持 H1（H1 接收条件 $\hat\rho_{\max}/\hat\rho_{\min}\geq 100$，**实测 3.92×10⁶≫100**，且抗异常点极差 $4.31×10^5$、分段中位数严格单调）与 H4（H4 接收条件 B1 偏小的 epoch 占比 ≥ 70%，**实测 91.3%**）。H1、H4 在本章即满足接收条件，H2、H3 的验证留待第 5 章。
