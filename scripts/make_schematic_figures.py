...
def fig_1_2_roadmap():
    ...
    ax.text(5.0, 0.55,
            "注：实线箭头为阶段推进顺序，细线箭头为各阶段输出；虚线表示核心纪律对各阶段的约束关系。"
            "中间任一环节缺失时，对应的创新表述不进入结论。",
    ...
    save(fig, "F1_2_technical_roadmap",
         {"figure_id": "1-2", "caption": "全文技术路线图", "type": "schematic",
          ...
          "revision": "对齐 THESIS_OUTLINE 六章版：移除旧七章体系的研究假设 H1–H5 与 GAAW(M1) 主创新表述，"
                      "核心问题框改为可追溯性纪律"})
...
def fig_1_3_chapter_map():
    ...
    chaps = [
        (0.35, 5.30, 3.1, 1.15, "第 1 章  绪论\n问题定位与研究边界", C_BOX, C_EDGE),
        (0.35, 3.70, 3.1, 1.15, "第 2 章  相关理论与研究进展\n方法谱系与可追溯依据", C_BOX, C_EDGE),
        (4.45, 4.50, 3.1, 1.15, "第 3 章  基线复现与瓶颈诊断\n协议核验与实测瓶颈", C_HL, C_HLE),
        (4.45, 2.60, 3.1, 1.15, "第 4 章  改进的上采样网络\n结构级改进与机制假设", C_HL, C_HLE),
        (8.55, 3.55, 3.1, 1.15, "第 5 章  实验结果与分析\n对比、消融、鲁棒性、失败案例", C_BOX, C_EDGE),
        (8.55, 1.30, 3.1, 1.15, "第 6 章  总结与展望\n已被支持的结论与局限", C_GRAY, "#888"),
    ]
    for x, y, w, h, t, fc, ec in chaps:
        box(ax, x, y, w, h, t, fc=fc, ec=ec, fs=9.2, lw=1.8 if fc == C_HL else 1.4,
            bold=(fc == C_HL))

    # 主链：1 -> 2 -> 3 -> 4 -> 5 -> 6
    arrow(ax, (1.90, 5.30), (1.90, 4.85), lw=1.6)
    arrow(ax, (3.45, 4.28), (4.45, 5.08), lw=1.6)
    arrow(ax, (6.00, 4.50), (6.00, 3.75), lw=1.6)
    arrow(ax, (7.55, 3.18), (8.55, 4.13), lw=1.6)
    arrow(ax, (10.10, 3.55), (10.10, 2.45), lw=1.6)

    # 支撑与回流关系
    arrow(ax, (3.45, 5.88), (8.55, 4.55), color="#999", lw=1.1, ls="--", rad=-0.16)
    arrow(ax, (7.55, 5.08), (8.55, 4.60), color="#999", lw=1.1, ls="--")
    arrow(ax, (8.55, 3.90), (7.55, 3.30), color="#C9A227", lw=1.3, ls="--", rad=0.18)

    ax.text(6.0, 6.72, "图 1-3  研究内容与章节关系图", ha="center", fontsize=12.5,
            fontweight="bold")
    ax.text(0.35, 2.55, "实线：研究内容的推进顺序\n虚线（灰）：依据支撑关系\n虚线（黄）：实测结果对方法定型的反馈",
            fontsize=9.0, color="#555", va="top", linespacing=1.9)
    ax.text(6.0, 0.42,
            "注：高亮框为本文的两项核心工作章节；第 4 章的结构改进以第 3 章的实测瓶颈为设计约束，"
            "其最终形式由第 5 章的消融结果确定。",
            ha="center", fontsize=8.4, color="#555")
    save(fig, "F1_3_chapter_map",
         {"figure_id": "1-3", "caption": "研究内容与章节关系图", "type": "schematic",
          "chapter": "1.6", "n_chapters": 6})
